...
if WINDOWS and check_version(
    TORCH_VERSION, "==2.4.0"
):  # reject version 2.4.0 on Windows
    LOGGER.warning(
...
def select_device(device="", newline=False, verbose=True):
    ...
    s = f"Ultralytics {__version__} (forked from Ultralytics) 🚀 Python-{PYTHON_VERSION} torch-{TORCH_VERSION} "
...
def get_flops(model, imgsz=640):
    ...
    try:
        ...
        try:
            ...
            im = torch.empty(
                (1, p.shape[1], stride, stride), device=p.device, dtype=p.dtype
            )  # input image in BCHW
            flops = (
            ...
            im = torch.empty(
                (1, p.shape[1], *imgsz), device=p.device, dtype=p.dtype
...
def profile_ops(input, ops, n=10, device=None, max_num_obj=0):
    ...
    for x in input if isinstance(input, list) else [input]:
        ...
        for m in ops if isinstance(ops, list) else [ops]:
            ...
            try:
                flops = (
                    thop.profile(deepcopy(m), inputs=[x], verbose=False)[0] / 1e9 * 2
                    if thop
                    else 0
                )  # GFLOPs
            except Exception:
            ...
            try:
                ...
                for _ in range(n):
                    ...
                    if (
                        max_num_obj
                    ):  # simulate training with predictions per image grid (for AutoBatch)
                        with cuda_memory_usage(device) as cuda_info:
                            torch.randn(
                                x.shape[0],
                                max_num_obj,
