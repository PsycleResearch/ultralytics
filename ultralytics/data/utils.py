...
def verify_image_mask(args: tuple) -> tuple:
    ...
    try:
        ...
        if os.path.isfile(mask_file):
            ...
            assert (
                mask.shape[:2] == shape
            ), f"mask size {mask.shape[:2]} does not match image size {shape}"
            nf = 1
        else:
            nm = 1
...
def visualize_image_annotations(
    ...
    for x, y, w, h, label in annotations:
        color = tuple(
            c / 255 for c in colors(label, False)
        )  # Get and normalize an RGB color for Matplotlib
        rect = plt.Rectangle(
...
def add_polygon_background(data: dict) -> dict:
    ...
    if (
        nc == 1
    ):  # binary: bg=0, fg=1 (implicit); model uses BCE on a single output channel
        data["bg_class_idx"] = 0
    else:
