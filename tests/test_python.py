...
def test_predict_img(model_name):
    ...
    im = cv2.imread(
        str(SOURCE), flags=cv2.IMREAD_GRAYSCALE if channels == 1 else cv2.IMREAD_COLOR
    )  # uint8 numpy array
    assert (
    ...
    assert (
        len(model(torch.rand((2, channels, 32, 32)), imgsz=32)) == 2
    ...
    assert (
        len(model(torch.zeros(320, 640, channels).numpy().astype(np.uint8), imgsz=32))
        == 1
    )  # tensor to numpy
    batch = [
        str(SOURCE),  # filename
    ...
    assert len(model(batch, imgsz=32, classes=0)) == len(
        batch
    )  # multiple sources in a batch
...
def test_track_stream(model):
    ...
    for gmc, reidm in zip(["orb", "sift", "ecc"], ["auto", "auto", "yolo11n-cls.pt"]):
        ...
        YAML.save(
            custom_yaml,
            {**default_args, "gmc_method": gmc, "with_reid": True, "model": reidm},
        )
...
@pytest.mark.skipif(
    IS_JETSON or IS_RASPBERRYPI, reason="Edge devices not intended for training"
)
...
def test_results(model: str):
    ...
    for r in results:
        ...
        r.to_df(
            decimals=3
        )  # Align to_ methods: https://docs.ultralytics.com/modes/predict/#working-with-results
        r.to_csv()
...
@pytest.mark.skipif(
    checks.IS_PYTHON_3_12, reason="YOLOE with CLIP is not supported in Python 3.12"
)
@pytest.mark.skipif(
    checks.IS_PYTHON_3_8 and LINUX and ARM64,
